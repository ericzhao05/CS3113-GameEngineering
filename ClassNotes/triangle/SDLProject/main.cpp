...
constexpr float HARDCODED_DELTA_TIME = 0.1f;
...
void initialise()
{
    ...
    // g_model_matrix = glm::translate(glm_model_matrix, glm::vec3(0.001, 0.1f, 0.1f))
    
    // rotate
    // glm::rotate(respect to what, 0.045f??, in what direction)
    // g_model_matrix = glm::rotate(g_model_matrix, 0.45f, glm::vec3(0.0f, 0.0f, 1.0f))
    
    // scaling
    // glm::scale(respect to what, glm::vec3(2.0f, 2.0f, 0.0f));
    // g_model_matrix = glm::scale(g_model_matrix, glm::vec3(2.0f, 2.0f, 0.0f));
...
void update()
{
//    g_model_matrix = glm::translate(g_model_matrix, glm::vec3(0.01f, 0.0f, 0.0f));
//    g_model_matrix = glm::rotate(g_model_matrix, 0.045f, glm::vec3(0.0f,0.0f,1.0f));
//    g_model_matrix = glm::scale(g_model_matrix, glm::vec3(1.001f, 1.001f, 0.0f));
    
    float ticks = (float) SDL_GetTicks()/100.0f;
    float delta_time = ticks - g_previous_ticks;
    g_previous_ticks = ticks;
    
    g_position.x += 0.5f * delta_time;
    
    g_model_matrix = glm::mat4(0.5f);
