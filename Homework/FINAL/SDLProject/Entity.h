...
public:
    ...
    bool is_dead() const { return m_lives <= 0; }
    ...
    const int* get_walking_indices(int direction) const { return (direction >= 0 && direction < 4) ? m_walking[direction] : nullptr; }
    const int* get_animation_indices() const { return m_animation_indices; }
    ...
    void const lose_life() { m_lives--; }
    void const set_lives(int new_lives) { m_lives = new_lives; }
    void const set_scene(Scene* scene) { m_scene = scene; }
    void const set_owner(Entity* owner) { m_owner = owner; }
    Entity* const get_owner() const { return m_owner; }


    int const get_enemy_lives() const { return m_enemy_lives; }
    void const set_enemy_lives(int new_lives) { m_enemy_lives = new_lives; }
    void const take_damage()
    { 
        m_enemy_lives--; 
        if (m_enemy_lives <= 0) 
        {
    ...
    bool const is_enemy_dead() const { return m_enemy_lives <= 0; }
