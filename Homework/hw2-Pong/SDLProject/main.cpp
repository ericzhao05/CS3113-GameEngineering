...
const float TOP_BOUND = 3.0f;
const float BOTTOM_BOUND = -3.0f;
const float LEFT_BOUND = -5.0f;
const float RIGHT_BOUND = 5.0f;

AppStatus g_app_status = RUNNING;
ShaderProgram g_shader_program = ShaderProgram();
glm::mat4 g_view_matrix, g_beaker_matrix, g_projection_matrix, g_drops_matrix, g_ball_matrix, g_court_matrix, g_left_winner_matrix, g_right_winner_matrix;
...
void update()
{
    ...
    if (WINNER.empty() == true){
        ...
        if (g_ball_position.y >= TOP_BOUND || g_ball_position.y <= BOTTOM_BOUND)
        {
            g_ball_movement.y = -g_ball_movement.y;
        }
        
        // collision on the left
        float x_distance_left = fabs(g_ball_position.x  - INIT_POS_DROPS.x) - ((INIT_SCALE_BALL.x + INIT_SCALE_DROPS.x) / 2.0f);
        float y_distance_left = fabs(g_ball_position.y - g_drops_position.y) - ((INIT_SCALE_BALL.y + INIT_SCALE_DROPS.y) / 2.0f);
        ...
        float x_distance_right = fabs(g_ball_position.x - INIT_POS_BEAKER.x) -((INIT_SCALE_BALL.x + INIT_SCALE_BEAKER.x) / 2.0f);
        float y_distance_right = fabs(g_ball_position.y - g_beaker_position.y) - ((INIT_SCALE_BALL.y + INIT_SCALE_BEAKER.y) / 2.0f);
    ...
    // End the game
    if (g_ball_position.x < LEFT_BOUND) {
        WINNER = "right";
        
    } else if (g_ball_position.x > RIGHT_BOUND) {
        WINNER = "left";
    }
}
