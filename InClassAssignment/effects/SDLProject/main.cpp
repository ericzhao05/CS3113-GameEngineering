...
void process_input()
{
    ...
    if (key_state[SDL_SCANCODE_C])
    {
        ...
        g_effects = new Effects(g_projection_matrix, g_view_matrix);
        g_effects->apply_shake_effect(g_view_matrix);
    }
    else
    {
...
void update()
{
    ...
    if (g_curr_scene->get_state().player->get_player_state() == CHARGING)
    {
        float random_x = ((float)rand() / RAND_MAX) * g_effects->MAX_OFFSET * 2.0f - g_effects->MAX_OFFSET;
        float random_y = ((float)rand() / RAND_MAX) * g_effects->MAX_OFFSET * 2.0f - g_effects->MAX_OFFSET;
        g_view_matrix = glm::translate(g_view_matrix, glm::vec3(random_x, random_y, 0.0f));
    }
}
