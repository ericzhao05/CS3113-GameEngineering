...
Effects::Effects(glm::mat4 projection_matrix, glm::mat4 view_matrix) :
    ...
    m_shake_offset_x(0.0f), m_shake_offset_y(0.0f)
{
    // Non textured Shader
...
void Effects::start(EffectType effect_type, float effect_speed)
{
    ...
    switch (m_current_effect)
    {
        ...
        case SHAKE:   
            ...
            m_shake_offset_x = 0.0f;
            m_shake_offset_y = 0.0f;
            break;
    }
}
...
void Effects::update(float delta_time)
{
   switch (m_current_effect)
   {
       ...
       case SHAKE:
           // Generate new random offset values for the shake effect
           m_shake_offset_x = ((float)rand() / RAND_MAX) * MAX_OFFSET * 2.0f - MAX_OFFSET;
           m_shake_offset_y = ((float)rand() / RAND_MAX) * MAX_OFFSET * 2.0f - MAX_OFFSET;
           break;
   }
}

glm::mat4 Effects::apply_shake_effect(glm::mat4 matrix)
{
    if (m_current_effect == SHAKE) {
        return glm::translate(matrix, glm::vec3(m_shake_offset_x, m_shake_offset_y, 0.0f));
    }
    return matrix;
}
...
void Effects::render()
{
    ...
    switch (m_current_effect)
    {
        ...
        case SHAKE:
            // Shake effect is applied to the view matrix in main.cpp using apply_shake_effect
            break;
    }
}
