...
void update(const int& randomNum)
{
    ...
    {
        if (g_state.player->check_collision(g_state.platforms, PLATFORM_COUNT))
        {
            g_state.player->update(FIXED_TIMESTEP, NULL, g_state.platforms, PLATFORM_COUNT);
            delta_time -= FIXED_TIMESTEP;
        } else if (g_state.player->check_collision(g_state.trap, 1))
        {
...
int main(int argc, char* argv[])
{
    ...
    g_state.trap = g_state.platforms[random_int].set_entity_type(TRAP);
